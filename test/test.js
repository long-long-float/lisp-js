...
(function() {
  describe('Lisp', function() {
    ...
    return describe('evaluator', function() {
      it('should treat nil as empty list', function() {
        expect(Lisp["eval"]("(car nil)").body).to.equal("nil");
        expect(Lisp["eval"]("(cdr nil)").body).to.equal("nil");
        return expect(Lisp["eval"]("(cons 10 nil)").body).to.equal("(10)");
      });
      describe('special forms', function() {
        ...
        it('should be able to define and call function', function() {
          return expect(Lisp["eval"]("(defun sum (x y) (+ x y))\n(sum 1 2)").body).to.equal('3');
        });
        it('should be able to assign value', function() {
          return expect(Lisp["eval"]("(setq a 10)\na").body).to.equal('10');
        });
        it('should be able to define and use macro', function() {
          return expect(Lisp["eval"]("(defmacro setnil! (var)\n  (list 'setq var nil))\n(setnil! a)\na").body).to.equal('nil');
        });
        return it('should be able to use valiable in let', function() {
          return expect(Lisp["eval"]("(let ((a 10)) a)").body).to.equal('10');
        });
      });
      return describe('basic functions', function() {
        it('should return sum of arguments', function() {
          return expect(Lisp["eval"]('(+ 1 2 3)').body).to.equal('6');
        });
        it('should return subtraction of arguments', function() {
          return expect(Lisp["eval"]('(- 1 2 3)').body).to.equal('-4');
        });
        it('should return multiplication of arguments', function() {
          return expect(Lisp["eval"]('(* 1 2 3)').body).to.equal('6');
        });
        it('should return division of arguments', function() {
          return expect(Lisp["eval"]('(/ 12 2 3)').body).to.equal('2');
        });
